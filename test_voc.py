...
def map_coco_to_voc(detections):
    mapped = []
    for det in detections:
        coco_id = det['class_id']
        if coco_id in COCO_TO_VOC:
            voc_id = COCO_TO_VOC[coco_id]
            det['class_id'] = voc_id
            mapped.append(det)
    return mapped
...
def test(data,
    ...
    # After model and dataset loaded/config
    names = {i: n for i, n in enumerate(VOC_NAMES)}   # <<<<< update to VOC names
    nc = 20   
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            # ========== MAPPING START ==========
            # Map predictions from COCO classes to Pascal VOC:
            mapped_preds = []
            for det in pred.cpu().numpy():
                ...
                if coco_id in COCO_TO_VOC:
                    voc_id = COCO_TO_VOC[coco_id]
                    det[5] = voc_id
                    mapped_preds.append(det)
            if not mapped_preds:
                if nl:
                    stats.append((torch.zeros(0, niou, dtype=torch.bool), torch.Tensor(), torch.Tensor(), tcls))
                continue
            predn = torch.tensor(mapped_preds, device=pred.device)
            # ========== MAPPING END ==========
